<script lang="ts">
	...
	// event handlers
	const onRmClick = async () => {
		const res = await contractActions.rmAdmin(addr);
		...
		toast.success('The address removed from admins');
		dispatch('close');
	};
</script>

<Modal title="Remove admin" on:close>
	<Input label="Address" bind:value={addr} error={addrErr} />

	<div class="flex gap-3 mt-4">
		<Button className="flex-grow" type="error" disabled={!!addrErr} on:click={onRmClick}>
			Remove
		</Button>
